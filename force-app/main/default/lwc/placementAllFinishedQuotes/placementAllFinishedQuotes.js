...
const COLUMNS = [
    ...
    { label: 'Responsável', fieldName: 'ownerName', type: 'text', hideDefaultActions: true },
...
export default class PlacementAllFinishedQuotes extends LightningElement {
            ...
            .then(success => {
                try {
                    ...
                    for (const key in queueNames) {
                        this.allQuotesRequest = [].concat(...queueNames.map(queueName => data.filter(quote => quote.ownerName.includes(queueName) && (quote.statusCase === 'Apólice registrada' || quote.statusCase === 'Processo anulado'))));
                        this.valuesChart = [].concat(...queueNames.map(queueName => data.filter(quote => quote.ownerName.includes(queueName) && (quote.statusCase === 'Apólice registrada' || quote.statusCase === 'Processo anulado'))));
                    }
